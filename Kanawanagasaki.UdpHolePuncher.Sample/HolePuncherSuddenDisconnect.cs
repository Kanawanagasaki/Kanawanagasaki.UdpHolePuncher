...
public class HolePuncherSuddenDisconnect
{
    public static async Task Main(string[] args)
    {
        ...
        await clientA.Start(TimeSpan.FromSeconds(2), default);
        ...
        await clientB.Start(TimeSpan.FromSeconds(2), default);

        while (clientB.PunchResult is null)
            await Task.Delay(100);

        var queryRes = await clientB.Query(["aaaaa"], TimeSpan.FromSeconds(1), default);
        if (queryRes is null || queryRes.PublicClients.Length == 0)
            Console.WriteLine("Query result or clients was null or length was 0");
        else
        {
            await clientB.Connect(queryRes.PublicClients[0], default);
            while (!clientB.ConnectedClients.Any())
                await Task.Delay(100);
            await clientB.SendTo(queryRes.PublicClients[0], Encoding.UTF8.GetBytes("I showed you my speedrun, please respond"), default);
        }
